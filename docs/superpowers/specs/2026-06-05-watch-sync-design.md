...
### 既知の制約（最終コードレビューで確認・post-MVPで対応）

- **ホストのページ再読込でルームが復帰しない**：ホストトークンはメモリ保持のみで永続読み戻しを実装していないため、再読込すると新規ルームを作成する（セッション内のWS再接続は復帰可。`chrome.storage` への書込は撤去済み）。
- **`seq` がホスト再読込でリセットされる**：長時間生存した参加者の `lastAppliedSeq` に対し、再読込したホストの `seq` が追いつくまで状態が無視されうる。恒久対応はホストepoch/セッションIDの導入。
- **再接続バックオフが安定接続後にリセットされない**：`attempt` は単調増加（コールドスタート対策としては正しい）。安定接続が一定時間続いたらリセットする改善は post-MVP。
- **join失敗（no_room）後の同一タブ再試行不可**：`started` ガードにより、ルーム未存在で失敗した後の再joinはタブ再読込が必要（ステータスは「ルームが存在しません」を表示）。
