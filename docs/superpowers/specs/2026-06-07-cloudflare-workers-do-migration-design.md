...
2. **無料の維持** — SQLite-backed DO は Workers Free プランで利用可能。無料枠は 100,000 req/日・13,000 GB-s/日・SQLite 5GB。2つの課金特性により本件は余裕で収まる：(a) **受信 WS メッセージは request 課金で 20:1 に圧縮**される（受信100＝5 request）、(b) **duration は active JS 実行時間（ハンドラ数ms）にのみ課金**され、hibernation-eligible な DO はメッセージ間の壁時計で課金されない。6人×5時間セッションでも ≈720 request / ≈9 GB-s で、無料枠に対し2桁の余裕がある（定量的根拠と維持条件は§5.2）。
...
加えて本移行で**新規の不変条件**を1つ導入する：

6. **DO を hibernation-eligible に保つ**（無料枠維持の生命線）。DO 内で `setInterval`/`setTimeout` を使わず（タイマーは Alarm へ）、未解決 Promise を残さず、WS は必ず `ctx.acceptWebSocket()` で受理する。これを破ると duration が壁時計課金化し無料枠が崩れる（経済的根拠と数値は§5.2）。`setInterval` 廃止（§5）・`acceptWebSocket`（§4.3）は機能要件であると同時にこの不変条件の実体である。
...
#### 5.2 経済性の根拠と hibernation-eligibility 不変条件

duration 課金は **DO の壁時計常駐ではなく、active JS 実行時間（ハンドラ数ms）にのみ**発生する。CF 公式："An idle Durable Object that qualifies for hibernation does not incur duration charges, even during the brief window before the runtime hibernates it."（実際に hibernate する前から非課金）。したがって heartbeat 5s 間隔でも、ハンドラが返った瞬間に DO は hibernation-eligible になり、**次メッセージまでの5秒間は壁時計が進んでも課金されない**。「再生中は実質常時アクティブ」という直感は課金モデル上は誤りで、過度に悲観的だった。

さらに**受信 WS メッセージは request 課金で 20:1 に圧縮**される。CF 公式："a 20:1 ratio is applied to incoming WebSocket messages to factor in smaller messages for real-time communication."（受信100メッセージ＝5 request 換算）。

実数（最悪寄り: 6人 × 5時間。heartbeat 5s 確定、ping は仮に10s 間隔・5s でも結論不変）：

```
受信メッセージ総数(5h): ホスト(heartbeat 3,600 + ping 1,800) + 参加者5×ping 1,800 ≈ 14,400 件
  request 課金(20:1): 14,400 / 20 ≈ 720 request   （無料枠 100,000/日 に対し約130回ぶん）
  duration 課金     : 14,400 × ~5ms × 0.128GB ≈ 9 GB-s（無料枠 13,000/日 に対し約1,400回ぶん）
```

→ 2〜5時間の長時間視聴でも無料枠に**2桁の余裕**。律速は視聴時間でも人数でもなく1日の累積メッセージ数で、開発者＋友人数名では到達しない。

**無料枠維持の不変条件（= hibernation-eligibility の維持）**：duration が数ms で済むのは DO がメッセージ間で hibernation-eligible を保つ前提に依存する。以下を破ると eligibility が失われ duration が**壁時計課金に化け**、5h 1ルーム = 0.128GB × 18,000s ＝ **2,304 GB-s（無料枠の約18%/セッション → 約5.6回/日で枯渇）**に跳ね上がる。

- (a) DO 内で `setInterval`/`setTimeout` を使わない（タイマーは Alarm へ。§5.1）
- (b) 未解決の Promise を残さない（ハンドラは必ず解決して返す）
- (c) WS は必ず `ctx.acceptWebSocket()` で受理する（素の `accept()` 不可。§4.3）

**つまり §5 の setInterval 廃止・§4.3 の `acceptWebSocket` は、機能要件であると同時に無料枠維持の生命線**であり、両者は別物ではなく同じ不変条件の表裏である。`lastState` の `storage.put` は sync/heartbeat 毎（再生中5s毎＋seek 毎）に走るが規模的に問題ない（§9）。
