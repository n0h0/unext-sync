...
# 2. dist/extension を unext-sync という名前でパッケージング
rm -rf dist/unext-sync && cp -R dist/extension dist/unext-sync
cd dist && zip -r ../unext-sync.zip unext-sync && cd ..
rm -rf dist/unext-sync
# → unext-sync.zip（解凍すると unext-sync/ フォルダができる）
```
この zip を私的経路で友人に渡す。
...
>
> 1. 受け取った `unext-sync.zip` を、**今後消さない場所**に解凍する（例: ドキュメント内に専用フォルダを作る）。
>    → 拡張はこのフォルダを直接読むため、**フォルダを消したり移動したりすると動かなくなる**。
> 2. Chrome のアドレスバーに `chrome://extensions` と入力して開く。
> 3. 画面右上の「**デベロッパー モード**」をオンにする。
> 4. 左上に出る「**パッケージ化されていない拡張機能を読み込む**」を押し、**手順1で解凍した `unext-sync` フォルダ**を選ぶ。
