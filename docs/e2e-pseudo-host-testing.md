...
## サーバー側 E2E スモーク（ヘッドレス・本番/ローカル両対応）

`scripts/e2e-prod-smoke.mjs` は **host と participant の両方を Node から接続**して、Worker+DO の
WS 同期リレーを実ブラウザなしで検証する自己完結スクリプト。デプロイ後の本番スモークとして再利用できる。

```bash
# 本番（既定の接続先は build.mjs と同じ workers.dev URL）
CONNECT_SECRET='<wrangler secret put で設定した値>' node scripts/e2e-prod-smoke.mjs

# ローカル wrangler dev に対して（.dev.vars の値と一致させる）
SERVER_URL=ws://localhost:8787 CONNECT_SECRET='<同値>' node scripts/e2e-prod-smoke.mjs
```

`CONNECT_SECRET` は Worker のシークレット（書き込み専用で読み戻せない）なので、`wrangler secret put` で
設定した値を env で渡す。検証項目（`9 passed, 0 failed` で全合格）:

1. `POST /create` → `{roomId, hostToken}`
2. ホスト join（role:host）
3. 途中参加者の join
4. **lastState キャッチアップ**（途中参加者が直近 state `currentTime=120` を受信）
5. **ライブリレー**（ホストの seek → 参加者が `state currentTime=300` を受信）
6. ping → pong（id echo）
7. ホスト切断 → 参加者へ `host_disconnected`
8. ホストがトークンで再接続 → ホストスロット奪還
9. 参加者へ `host_resumed`

**実績**: 2026-06-07 本番（`wss://unext-sync.kusakatsubasa-dba.workers.dev`）に対し **9 passed, 0 failed**。

これは**サーバー側リレーの自動検証**であり、下記の擬似ホスト＋実ブラウザ手順（参加者の `<video>`
実制御・popup 描画）を置き換えるものではない。両者を併用して全体を担保する。

## 仕組みの要点

- **接続ゲート**: `CONNECT_SECRET` は WebSocket サブプロトコルヘッダで送る → `new WebSocket(url, [secret])`。
  Worker は `Sec-WebSocket-Protocol` ヘッダで定数時間比較検証する（`worker/src/index.ts`）。Node `ws` でも第2引数で同様に送れる。
- **ホストスロット確保が必須**: サーバーは hostId 不一致の sync を破棄する（`shared/rooms.ts` reducer）。
  擬似ホストは必ず `POST /create`（→ `{roomId, hostToken}` 取得）→ `join(role:"host")` してから `sync` を送る。
...
# 1. ローカル Worker 起動（バックグラウンド）
#    CONNECT_SECRET は .dev.vars ファイルから読み込む（CONNECT_SECRET=<hex> を1行記述）
...
> 既定（未指定）では本番 `wss://unext-sync.<subdomain>.workers.dev` が埋め込まれるので、`extension/src/config.ts` を
...
（擬似ホストが 200ms ポーリング）。**起動時点でファイルに残っている `n` は基準値として扱われ、
そのコマンドは発火しない**（前回セッションのコマンドが起動直後に勝手に走らない）。起動後に `n` を
インクリメントしたぶんだけが実行される。このファイルは gitignore 済のローカル作業用で、リポジトリには
含まれない（初回は存在しないので何も発火しない）。手動で置くなら no-op の `{"n":0,"cmd":""}` から始めるとよい。
...
ユニットテスト（`shared/rooms.test.ts`, worker テストスイート）で実証済み。popup の**描画**を実機確認する。
...
ユニット/Worker テスト（`shared/rooms.test.ts`、worker テストスイート）と擬似ホスト＋Node参加者で実証済み。ここでは
popup の**描画**を確認する。
