...
2つのChromeプロファイルで `dist/extension` を読み込み、同一タイトルを開いて確認:
- [ ] ホストでルーム作成 → popupにルームID（`room_created`経由）が表示される。
- [ ] 参加者がそのIDで参加。
- [ ] ホストのplay/pause/seek/ratechangeが参加者に反映される。
- [ ] 参加者が手動でずらすと、最大5秒以内（または `seeking` 即時）でホスト位置に戻る。
- [ ] ホストのタブを再読み込み（切断→再接続）→ 参加者statusが「ホスト切断」→「接続済み」へ。
- [ ] **クロックスキュー回帰**：片方のPCの時計を数分ズラしても同期がズレないことを確認（壁時計非依存設計の検証）。
- [ ] Renderコールドスタート：しばらく放置後の初回接続で、再接続バックオフにより数十秒以内に接続できる。
